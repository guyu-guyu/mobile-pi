...
**同步基线（Sync Baseline）**：
托管工作区与授权目录最近一次确认一致时的文件状态，用于识别两侧之后发生的独立变化。
_避免使用_：缓存、当前快照

**同步计划（Sync Plan）**：
Mobile Pi 在比较同步基线、托管副本和授权目录后提出的一组导入、写回、删除与冲突项；计划本身不会修改文件。
_避免使用_：自动同步、文件队列

**同步冲突（Sync Conflict）**：
同一路径在托管副本和授权目录中分别偏离同步基线，且两侧内容无法自动收敛的状态。
_避免使用_：同步错误、覆盖提示
...
**未绑定诊断终端（Unbound Diagnostic Terminal）**：
不属于任何会话或工作区、只用于检查内嵌运行环境的交互式终端。
_避免使用_：空会话、无会话 Agent、全局会话
